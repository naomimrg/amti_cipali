    ...
    <div class="row">
        <div class="col-6">
            <h4 class="black-color"><a style="color:black!important;" href="../">{{$vendor->nama_vendor}}</a> - <a
                    style="color:black!important;" href="./">{{$lokasi->nama_lokasi}} ini kan yang di hapus</a></h4>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="" placeholder="Masukkan Nama Span"
                            name="nama_span" id="nama_span">
                    </div>
                    <div class="form-group">
                        <label><b>Station ID</b></label>
                        <input type="text" class="form-control" value="" placeholder="Masukkan Station ID"
    ...
    function showData() {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log("ini data span " + JSON.stringify(data));
                $.each(data.items, function(index, item) {
                    console.log(item);
                    $('#list-span').append(
                        '<div class="col-4"><div class="loc-list"><a href="{{ url("/vendor")}}/' +
                        idVendor + '/' + id + '/live_sensor/' + item.id +
                        '"><img src="{{ url("/assets") }}/img/span/' + item.foto +
                        '"></a><div class="btn-group" style="position: absolute;"><button style="background: #545c58ab;border: none;position: absolute;top: -16px;right: -16px;" type="button" class="btn btn-primary btn-icon rounded-pill dropdown-toggle hide-arrow" data-bs-toggle="dropdown" aria-expanded="false"><i class="bx bx-dots-vertical-rounded"></i></button><ul class="dropdown-menu dropdown-menu-end"><li><a class="dropdown-item" style="padding: 2px 4px;"><button type="button" data-id="' +
                        item.id +
                        '" data-action="edit" class="action btn btn-sm" data-toggle="tooltip" title="Ubah"><i class="bx bxs-edit"></i>&nbsp; Ubah</button></a><li><a class="dropdown-item" style="padding: 2px 4px;"><button type="button" data-id="' +
                        item.id +
                        '" data-action="hapus" class="action btn btn-sm" data-toggle="tooltip" title="Delete"><i class="bx bxs-trash"></i>&nbsp; Hapus</button></a></li></ul></div><center><h4 class="black-color mb-0 mt-2" style="background: #0afeff;padding: 5px 5px;border-radius: 10px;">' +
                        item.nama_span + '</h4></center></div></div>');
                });
            }
        });
    }
