        ...
        <div class="row">
            <div class="col-7">
                <h4 class="black-color"><a style="color:black!important;" href="../">{{ $vendor->nama_vendor }}</a> - <a
                        style="color:black!important;" href="./">{{ $lokasi->nama_lokasi }} </a>
                    <!-- - {{ $span->nama_span }} -->
                </h4>
            </div>
...
@section('script')
    <script>
        ...
        const SAMPLE_INTERVAL = 3_000;
        const POLL_INTERVAL = 3_000;
        const MAX_RETRY = 5;
        const AUTO_RESET_EVERY_WINDOW = false;

        const MAX_POINTS = Math.ceil(WINDOW_MS / SAMPLE_INTERVAL);

        let chart, timeStamps = [];
        ...
        function parseDT(datetime) {
            if (!datetime) return Date.now();
            if (typeof datetime === 'string' && datetime.includes(' ')) datetime = datetime.replace(' ', 'T');
        ...
        function setYAxisByType(txt) {
            let y = 'Acceleration';
            const t = (txt || '').toLowerCase();
            if (t.includes('displacement')) y = 'Displacement';
            else if (t.includes('tiltmeter')) y = 'Degree';
            else if (t.includes('strain')) y = 'Microstrain';
        ...
        function pushPoint(ts, val) {
            ...
            if (lastPlottedTs === bucketTs && timeStamps.length) {
                chart.data.datasets[0].data[chart.data.datasets[0].data.length - 1] = val;
                chart.update();
                return;
            }
            ...
            chart.data.labels.push(fmt(bucketTs));
        ...
        function updateStatusUI(obj) {
            ...
            if (s) s.style.backgroundColor = color || status || 'black';
            if (v) v.innerHTML = `Current Value = ${value} ${satuan||''}`;
        }

        function extractHistoryArray(p) {
            if (!p) return [];
            if (Array.isArray(p)) return p;
            for (const k of ['history', 'data', 'items', 'rows', 'result'])
                if (Array.isArray(p[k])) return p[k];
            if (p.datetime !== undefined && p.value !== undefined) return [p];
            return [];
        }
        ...
        async function loadInitialWindow(sensorId, sensorTypeText) {
            ...
            try {
                const url = buildUrl(sensorId, `range_ms=${WINDOW_MS}&mode=history&step_ms=${SAMPLE_INTERVAL}`);
                const payload = await fetchJSON(url);
                const arr = extractHistoryArray(payload)
                    .map(d => ({
                        ts: parseDT(d.datetime),
                        value: d.value,
                        status: d.status,
                        satuan: d.satuan,
                        color: d.color
                ...
                arr.forEach(d => pushPoint(d.ts, d.value));
                if (arr.length) updateStatusUI(arr[arr.length - 1]);
                enforceWindowSize();
                retryCount = 0;
            } catch (e) {
                console.error(e);
                if (++retryCount <= MAX_RETRY) {
                    setTimeout(() => loadInitialWindow(sensorId, sensorTypeText), Math.min(5000, 500 * retryCount));
                }
            }
        }

        async function updateChart() {
            const sel = document.getElementById('sensor_id');
            if (!sel || !sel.value) return;
            const sensorId = sel.value;
            const sensorTypeText = sel.options[sel.selectedIndex]?.text || '';

            if (lastSensorId !== sensorId) {
                lastSensorId = sensorId;
                await loadInitialWindow(sensorId, sensorTypeText);
                return;
            }

            try {
                const data = await fetchJSON(buildUrl(sensorId));
                const ts = parseDT(data.datetime);
                setYAxisByType(sensorTypeText);
        ...
        function ensureTimers() {
            ...
            if (AUTO_RESET_EVERY_WINDOW) {
                resetTimer = setInterval(() => {
                    const sel = document.getElementById('sensor_id');
                    if (!sel || !sel.value) return;
                    loadInitialWindow(sel.value, sel.options[sel.selectedIndex]?.text || '');
                }, WINDOW_MS);
            }
        }
        ...
        (function() {
            ...
            if (sel && sel.value) {
                lastSensorId = sel.value;
                loadInitialWindow(lastSensorId, sel.options[sel.selectedIndex]?.text || '');
            }
            ...
            if (sel) {
                sel.addEventListener('change', async () => {
                    ...
                    lastSensorId = sel.value;
                    await loadInitialWindow(lastSensorId, sel.options[sel.selectedIndex]?.text || '');
                    ensureTimers();
                });
            }
