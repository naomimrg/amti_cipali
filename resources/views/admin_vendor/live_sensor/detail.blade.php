    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.items, function (index, item) {
                let sensorName = customSensorNames[item.sensor_id] || item.sensor_id; // Gunakan custom name jika tersedia
                $("#sensor_id").append('<option value="' + item.id + '">' + sensorName + '</option>');
            });
        }
    });
}
...
showData();
    ...
    let sensorLabel = customSensorNames[data.sensor_id] || data.sensor_id;

    chart.data.labels.push(data.datetime);
    chart.data.datasets[0].label = sensorLabel;
